...
def resolve_dataset_path(path: Path) -> Path:
    ...
    search_roots: List[Path] = [Path.cwd(), script_dir]
    if repo_root:
        search_roots.append(repo_root)
        main_dir = repo_root / "main"
        if main_dir.exists():
            search_roots.append(main_dir)
    ...
    for root in search_roots:
        ...
        if len(expanded.parts) > 1:
            candidates.append(root / expanded.name)
    ...
    if repo_root and repo_root.exists():
        available = sorted({p.name for p in repo_root.glob("*.csv")})

    checked_locations = ", ".join(str(item) for item in checked)
    availability_hint = (
        f" Available datasets: {', '.join(available)}" if available else ""
    )
    raise FileNotFoundError(
        f"Dataset not found: {path}. Checked: {checked_locations}.{availability_hint}"
    )
...
def load_dataset(path: Path) -> Tuple[List[dict], Path]:
    ...
    *,
    ...
    if test_data:
        for dataset_path in test_data:
            try:
                alternate_rows, resolved_alternate = load_dataset(dataset_path)
            except FileNotFoundError as error:
                missing_external.append(str(error))
                continue
            ...
            external_reports.append(
                EvaluationReport(
                    label=str(resolved_alternate.name),
...
def print_report(
    *,
    ...
    if result.missing_external:
        print("The following external datasets could not be evaluated:")
        for detail in result.missing_external:
            print(f"  - {detail}")
